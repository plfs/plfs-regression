...
// Test 1:  Create file
if((plfs_open(&pfd, path, O_CREAT|O_TRUNC|O_RDWR, 99, 0666, NULL)) < 0 )
{
...
if((ret_value = plfs_write(pfd, write_buf1, 6, 0, 99)) != 6 ) //6 bytes at offset 0
{
...
if((ret_value = plfs_write(pfd, write_buf2, 5, 6, 99)) != 5 ) //5 bytes at offset 6
{
...
// Test 4:  close file 
if((plfs_close(pfd, 99, 99, O_CREAT|O_TRUNC|O_RDWR, NULL)) < 0 )
{
...
// Test 5:  Open File
pfd = NULL;
if((plfs_open(&pfd, path, O_RDWR, 99, 0666, NULL)) < 0 )
{
...
if((ret_value = plfs_read(pfd, read_buf, 11, 0)) != 11 )
{
...
// Test 7: close file
if((plfs_close(pfd, 99, 1, O_RDWR, NULL)) < 0 )
{
