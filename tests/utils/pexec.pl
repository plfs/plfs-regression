...
sub CacheNetgroups ($) {
   ...
   foreach my $key (sort keys %TMP) {
      my %SEEN = ();
      my @Array = &ExpandNetgroup(\%TMP, $TMP{$key});
      @{ $href->{$key} } =			# guarantee unique array elements
	 grep { ! $SEEN{ $_ }++ } @Array;	# for netgroup intersection
   }						# calculation below
}
...
sub IntersectNetgroups ($$) {
   ...
   foreach my $member (@{ $aref1 }, @{ $aref2 }) {
      $SEEN{$member}++;
   }

   foreach my $member (keys %SEEN) {
      push (@Intersect, $member)
	 if ($SEEN{$member} > 1);		# member is in BOTH arrays
   }
...
M:    foreach (split(',',$machines)) {
	 ...
	 if (/^\@(.*)$/) {				# a lone netgroup
	    ...
	    next M;
	 } elsif (/\@/) {				# intersection of netgroups
	    my @Isect = ();
	    my $first_time = 1;
	    foreach my $ng (split('@')) {
	       if (defined($href->{$ng})) {
	    ...
	    push(@Machines,@Isect);
	    next M;
	 } elsif (/^\^(.*)$/) {				# a file containing
	    unless (open(FILE, $1)) {			# hosts in the first
	       push(@Machines,$_);			# column, optionally
   ...
   my %SEEN = ();
   my @Unique = grep { ! $SEEN{ $_ }++ } @Machines;	# weed out duplicates
   return @Unique;
}
